...
namespace VG.CDF.Server.Infrastructure.Services
{
    public class TagsGroupService : ITagsGroupService
    {
        ...
        public async Task Save(TagsGroupRequestDto entity)
        {
            using var db = new SqlDataContext(_dbConnectionConfig);
            var tagsGroupRepository = new TagsGroupRepository(db);
            await tagsGroupRepository.Save(_mapper.Map<ParameterValuesGroup>(entity));
        }
        ...
        public async Task SaveTagsGroups(List<TagsGroupRequestDto> tagsLiveGroups)
        {
            using var db = new SqlDataContext(_dbConnectionConfig);
            var tagsGroupRepository = new TagsGroupRepository(db);
            var tagsGroupEntity = _mapper.Map<List<ParameterValuesGroup>>(tagsLiveGroups);
            await tagsGroupRepository.SaveGroups(_mapper.Map<List<ParameterValuesGroup>>(tagsGroupEntity));

        }

        public async Task SaveTagsLiveGroups(List<TagsLiveGroupRequestDto> tagsLiveGroups)
        {
            var tagsGroupList = new List<ParameterValuesGroup>();
            using var db = new SqlDataContext(_dbConnectionConfig);
            var tagsGroupRepository = new TagsGroupRepository(db);
            foreach (var tagsGroup in tagsLiveGroups)
            {
                var tagsGroupEntity = _mapper.Map<ParameterValuesGroup>(tagsGroup.TagsGroup);
                var tagsLiveEntity = _mapper.Map<List<ParameterValue>>(tagsGroup.TagsLive);
                foreach (var tag in tagsLiveEntity)
                {
                    tagsGroupEntity.ParameterValues.Add(tag);
                }
                
                tagsGroupList.Add(tagsGroupEntity);
            }

            await tagsGroupRepository.SaveGroups(_mapper.Map<List<ParameterValuesGroup>>(tagsGroupList));
        }
        ...
        public async Task<TagsGroupResponseDto> Update(long id, TagsGroupRequestDto entity)
        {
            using var db = new SqlDataContext(_dbConnectionConfig);
            var tagsGroupRepository = new TagsGroupRepository(db);
            var result = await tagsGroupRepository.Update(id, _mapper.Map<ParameterValuesGroup>(entity));
